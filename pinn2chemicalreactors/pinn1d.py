...
class pinn1d:
    ...
    def __init__(self, problem, X, u, Xf, layers, lb, ub, Ymin, Ymax, options, teta_guess, kappa_min, kappa_max,  weights, biases, from_scratch, reactor):
        ...
        self.x1f_tf = tf.placeholder(tf.float32, shape=[None, self.x1f.shape[1]])

        # Neural networks: predict solution and residuals
        self.u_pred = self.net_u(self.x1_tf)
        self.f_pred = self.net_f(self.x1f_tf)
    ...
    def net_u(self, x1):
        u = self.neural_net(tf.concat([x1],1), self.weights, self.biases)
        return u
    ...
    def train(self,nIter):
        ...
        tf_dict = {self.x1_tf: self.x1, self.u_tf: self.u, self.x1f_tf: self.x1f}
    ...
    def predict(self, X_star):
        ...
        tf_dict = {self.x1_tf: X_star[:,0:1], self.x1f_tf: X_star[:,0:1]}
    ...
    def net_f_AcetoAcetylation(self, t):
        ...
        # Residence time
        tau = self.reactor['V']/self.reactor['Qin']

        # Reaction rates
        r1 = kappa[0]*CA*CB*self.reactor['Ccat']
        r2 = kappa[1]*CB*CB*self.reactor['Ccat']
        r3 = kappa[2]*CB
    ...
    def net_f_AllylChlorination(self, V):
        ...
        E1 = kappa[2]
        E2 = kappa[3]
        k1 = A1*tf.exp(-E1/8.314/self.reactor['T'])
        k2 = A2*tf.exp(-E2/8.314/self.reactor['T'])
